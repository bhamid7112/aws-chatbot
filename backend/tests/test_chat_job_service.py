...
class Harness(NamedTuple):
    """Both halves of the use case, plus the doubles they were built on."""

    service: ChatJobService
    runner: ChatJobRunner
    store: FakeJobStore
...
def build(
    ...
    **kwargs: object,
    ...
    chat = ChatService(generator or FakeReplyGenerator())

    # Settings belong to whichever half owns them, so route each one to the
    # constructor that takes it. Passing a runner's setting to the service (or
    # vice versa) is now a TypeError, which is the point of the split.
    runner_keys = {"flush_chars", "flush_interval_seconds", "max_reply_chars"}
    runner_kwargs = {k: v for k, v in kwargs.items() if k in runner_keys}
    service_kwargs = {k: v for k, v in kwargs.items() if k not in runner_keys}

    # Flush per chunk unless a test says otherwise, so that "what was written"
    # is legible rather than dependent on timing.
    runner_kwargs.setdefault("flush_chars", 1)
    ...
    return Harness(service, runner, resolved_store, resolved_dispatcher, resolved_clock)


class TestSubmit:
    async def test_records_the_job_before_handing_it_off(self) -> None:
        service, _, store, dispatcher, _ = build()
    ...
    async def test_the_deadline_covers_handoff_and_run(self) -> None:
        service, _, store, _, clock = build(deadline_seconds=120)
...
class TestRun:
    async def test_writes_the_reply_and_finishes(self) -> None:
        service, runner, store, _, _ = build(
            generator=FakeReplyGenerator(["Hi ", "there"])
        )
        job_id = await service.submit(REQUEST)

        await runner.run(job_id, REQUEST)
    ...
    async def test_a_second_delivery_of_the_same_job_does_nothing(self) -> None:
        ...
        service, runner, store, _, _ = build(generator=generator)
        job_id = await service.submit(REQUEST)
    ...
    async def test_each_append_says_what_length_it_expects(self) -> None:
        service, runner, store, _, _ = build(generator=FakeReplyGenerator(["a", "b"]))
        job_id = await service.submit(REQUEST)
    ...
    async def test_a_real_defect_is_recorded_and_re_raised(self) -> None:
        ...
        service, runner, _, _, _ = build(store=store)
        job_id = await service.submit(REQUEST)
...
class TestRetriedAppend:
    ...
    async def test_re_sending_a_committed_append_does_not_duplicate_text(
        ...
        store = FakeJobStore()
        service, _, _, _, _ = build(store=store)
        job_id = await service.submit(REQUEST)
    ...
    async def test_the_writer_resynchronises_and_carries_on(self) -> None:
        ...
        assert runner._continue_after(job_id, result) is True
...
class TestDeadline:
    async def test_a_job_past_its_deadline_reads_as_failed(self) -> None:
        ...
        service, _, _, _, clock = build(deadline_seconds=60)
        job_id = await service.submit(REQUEST)
    ...
    async def test_healing_does_not_write(self) -> None:
        ...
        service, _, store, _, clock = build(deadline_seconds=60)
        job_id = await service.submit(REQUEST)
    ...
    async def test_a_finished_job_is_never_healed(self) -> None:
        service, runner, _, _, clock = build()
        job_id = await service.submit(REQUEST)
        await runner.run(job_id, REQUEST)
...
class TestCancel:
    ...
    async def test_will_not_overwrite_a_finished_reply(self) -> None:
        ...
        service, runner, store, _, _ = build(generator=FakeReplyGenerator(["done"]))
        job_id = await service.submit(REQUEST)
        await runner.run(job_id, REQUEST)
